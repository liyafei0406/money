/*绿色下拉菜单 开始*/
$('.wordsbox').hover(function(){
    index=$(this).index();
    console.log(index);
    $('.greenmenu').eq(index).stop().slideDown('slow')
},function(){
        $('.greenmenu').eq(index).stop().slideUp('slow');
    });
...
$('.allcheck').click(function(){
    if ($(this).get(0).checked) {
        $('.onecheck').each(function(){this.checked=true});
        // $('.onecheck').each(function(){$(this).attr('checked',true)});
    } else {
        $('.onecheck').each(function(){this.checked=false});
        // $('.onecheck').each(function(){$(this).attr('checked',false)});
    }
})
...
$('.onecheck').click(function(){
    ...
    if (this.checked) {
        ...
        $('.subtract').eq(index).click(function(){
            var pcs=parseInt($('.pcs').text());
            $('.pcs').text(pcs-pce);
        }) 
    } else{
        var num=parseInt($('.numberbox').eq(index).text());
        var pcs=parseInt($('.pcs').text());
         $('.pcs').text(pcs-num);
         if ($('.pcs').text(pcs-num)<=0) {
            $('.pcs').text(0);
         } 
    }
})
...
$('.downmenu').click(function(){
    $('.btnstyle').text($(this).text());
})
$('.downmenu').mouseover(function(){
    $('.downmenu').css('backgroundColor','white');
    $(this).css('backgroundColor','#ebeae4')
})
/*number  开始*/
...
$('.add').click(function(){ 
    ...
    console.log(index);
    var num=parseInt($('.numberbox').eq(index).text());
    var num1=parseInt($('.surplus span').eq(index).text());
    var num2=parseInt($('.oneprice').eq(index).text());
    num1--;
    if (num1<0) {
        return false;
    }
    $('.numberbox').eq(index).text(num+1);
    $('.surplus span').eq(index).text(num1);
    $('.total').eq(index).text(num2*(num+1)+'.00')
})
